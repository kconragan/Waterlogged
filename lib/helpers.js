...
/**
 * Return a .txt file of buoy readings from NOAA
 *
 * @param {String} buoy         -> An NDBC buoy identifier (eg. 46237)
 * @param {Date} timeOfInterest -> The date/time we want the buoy reading for
 */
var getBuoyData = function(buoyId, timeOfInterest) {
  ...
  // use the 45-day data
    ...
    var url = 'http://www.ndbc.noaa.gov/data/realtime2/' + buoyId + '.txt';
  }
  // use the stdmet monthly data (only calculated at the *end* of each month)
    ...
    var url = 'http://www.ndbc.noaa.gov/data/stdmet/' + month + '/' + buoyId + '.txt';
  }
  // use yearly archives going back to 2007
    ...
    var url = 'http://www.ndbc.noaa.gov/view_text_file.php?filename=' + buoyId + 'h' + year + '.txt.gz&dir=data/historical/stdmet/';
    console.log('fetch yearly data', url);
  }
  else {
    var url = null;
    console.log('could not get a valid url');
  }
...
/**
 * Parse buoy data returned from NOAA
 *
 * @param {String} buoy   -> An NDBC buoy identifier (eg. 46237)
 * @param {Date} seshDate -> The date/time we want the buoy reading for
 */
var parseBuoyData = function(buoy, seshDate) {
  // parse ndbc buoy readings and find best match.

  // grab raw buoy readings from NOAA
  return getBuoyData(buoy, seshDate).then(function(data) {
