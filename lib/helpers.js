...
var parseTideAndWind = function(date, location) {
  ...
  return getTideAndWind(date, location).then(function(data) {
    ...
    var smallestTideDelta = null
    var closestTideObs = null;
    var tideIndex = null;
    ...
    if(tides.length) {
      ...
      for(i=0; i < tides.length; i++) {
        ...
        var delta = timeDelta(sessionTime, tideDate);

        if(!smallestTideDelta || Math.abs(delta) < smallestTideDelta) {
          smallestTideDelta = Math.abs(delta);
    ...
    // TODO: make sure this exsists first
